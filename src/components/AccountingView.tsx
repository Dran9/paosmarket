...
export default function AccountingView() {
  ...
  const mTx = transactions.filter(t => new Date(t.date) >= thisMonth);
  const mExp = expenses.filter(e => new Date(e.date) >= thisMonth);
  const totalIncome = round2(mTx.reduce((s, t) => s + t.total, 0));
  const totalCOGS = round2(mTx.reduce((s, t) => s + t.items.reduce((ss, i) => ss + i.cost * i.qty, 0), 0));
  ...
  const ivaCollected = round2(mTx.reduce((s, t) => s + t.tax, 0));
  ...
  const handleExport = () => {
    ...
    const txData = transactions.map(t => ({ Ticket: t.id, Fecha: fmtDateTime(t.date), Metodo: t.method, Items: t.items.length, Subtotal: t.subtotal, IVA: t.tax, Total: t.total, Atiende: t.attendedBy }));
    const ws1 = XLSX.utils.json_to_sheet(txData);
    ws1['!cols'] = [{ wch: 12 }, { wch: 22 }, { wch: 12 }, { wch: 6 }, { wch: 12 }, { wch: 10 }, { wch: 12 }, { wch: 16 }];
    XLSX.utils.book_append_sheet(wb, ws1, 'Ingresos');
    const expData = expenses.map(e => ({ ID: e.id, Fecha: fmtDate(e.date), Categoria: e.category, Descripcion: e.description, Monto: e.amount }));
    const ws2 = XLSX.utils.json_to_sheet(expData);
    XLSX.utils.book_append_sheet(wb, ws2, 'Egresos');
    XLSX.writeFile(wb, `Paolitas_Contabilidad_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={handleExport} className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold rounded-lg transition-all"><i className="fas fa-file-excel mr-2" />Exportar a Excel</button>
      </div>
      ...
      <div className="bg-white rounded-xl border p-5 mb-5">
        <h3 className="font-bold text-sm mb-3">Desglose Matematico (Mes Actual)</h3>
          ...
          <div className="bg-white rounded-xl border p-5"><h3 className="font-bold text-sm mb-3">Ingresos Recientes</h3>
            <div className="max-h-80 overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{transactions.slice(0, 15).map(t => (
                  <tr key={t.id} className="border-t border-slate-100"><td className="py-2 font-bold">{t.id}</td><td className="py-2 text-xs">{fmtDateTime(t.date)}</td><td className="py-2 text-xs">{t.attendedBy}</td><td className="py-2 text-right font-bold text-emerald-600">+{fmt(t.total)}</td></tr>
                ))}</tbody>
              </table>
            </div>
          </div>
          <div className="bg-white rounded-xl border p-5"><h3 className="font-bold text-sm mb-3">Gastos Recientes</h3>
            <div className="max-h-80 overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody>{expenses.slice(0, 15).map(e => (
      ...
      {tab === 'ingresos' && (
        <div className="bg-white rounded-xl border overflow-hidden">
          <div className="p-4 border-b"><h3 className="font-bold">Historial de Ingresos</h3></div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setShowNewExp(false)}>
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-[90%] p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={() => { if (!newExp.description || !newExp.amount) return; addExpense({ date: new Date().toISOString(), category: newExp.category, description: newExp.description, amount: parseFloat(newExp.amount) || 0 }); setShowNewExp(false); setNewExp({ category: 'Renta', description: '', amount: '' }); }} className="px-5 py-2 text-sm font-bold bg-indigo-500 text-white rounded-lg"><i className="fas fa-save mr-1" />Guardar</button>
            </div>
          </div>
        </div>
      )}
...
function EditableCell({ value, onSave, isNumber }: { value: string; onSave: (v: string) => void; isNumber?: boolean }) {
  ...
  return <span onDoubleClick={() => setEditing(true)} className="editable-cell">{value}</span>;
}
