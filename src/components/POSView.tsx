...
import {
  Search, ShoppingCart, Trash2, ShoppingBasket, X, Store, Truck,
  CreditCard, Banknote, QrCode, Landmark, CheckCircle, Check,
  Receipt, Coins, User, Phone, Printer,
} from 'lucide-react';
...
export default function POSView() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-5 h-[calc(100vh-48px)]">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setPosCategory(c)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-semibold border transition-all ${posCategory === c ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300'}`}
              style={posCategory === c ? {} : { color: CAT_COLORS[c] }}>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {cart.length === 0 ? (
            ...
            <div key={i} className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-indigo-50/50 transition-colors">
              ...
              <button onClick={() => removeFromCart(i)} className="text-slate-300 hover:text-red-500 transition-colors ml-1">
                <X size={12} />
              </button>
            </div>
          ))}
        ...
        <div className="p-4 border-t bg-slate-50">
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            <button onClick={() => setSaleType('site')}
              className={`flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all ${saleType !== 'delivery' ? 'bg-indigo-500 text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>
              <Store size={12} /> En Sitio
            </button>
            <button onClick={() => setSaleType('delivery')}
              className={`flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all ${saleType === 'delivery' ? 'bg-amber-500 text-white' : 'bg-white text-slate-500 border border-slate-200'}`}>
              <Truck size={12} /> Entrega
            </button>
          </div>
          {saleType === 'delivery' && (
            <div className="bg-amber-50 rounded-lg px-3 py-1.5 mb-2 text-[11px] font-semibold text-amber-600 text-center flex items-center justify-center gap-1">
              <Truck size={11} /> MODO ENTREGA
            </div>
          )}
          ...
          <button disabled={cart.length === 0}
            ...
            {saleType === 'delivery' ? <Truck size={15} /> : <CreditCard size={15} />}
      ...
      {/* Payment modal */}
      {showPayment && (
        <Modal onClose={() => setShowPayment(false)} title="Procesar Pago" Icon={CreditCard}>
          ...
          <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Método de pago</label>
          <div className="grid grid-cols-3 gap-3 mb-5">
            {([
              { m: 'Efectivo', Icon: Banknote },
              { m: 'QR',       Icon: QrCode },
              { m: 'Mixto',    Icon: Landmark },
            ] as const).map(({ m, Icon: MIcon }) => (
              <button key={m} onClick={() => setPayMethod(m)}
                ...
                <MIcon size={24} className="mx-auto mb-1" />
          ...
          {payMethod === 'QR' && (
            <div className="text-center py-5">
              ...
              <div className="p-3 bg-emerald-50 rounded-lg text-emerald-600 font-semibold text-sm flex items-center justify-center gap-1.5">
                <CheckCircle size={14} /> QR simulado — Confirmar para continuar
              </div>
            </div>
          )}
          {payMethod === 'Mixto' && (
      ...
      {/* Delivery modal */}
      {showDelivery && (
        <Modal onClose={() => setShowDelivery(false)} title="Pedido con Entrega" Icon={Truck}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Teléfono" value={dlClient.phone} onChange={v => setDlClient({ ...dlClient, phone: v })} placeholder="591..." />
            <Field label="Zona" value={dlClient.zone} onChange={v => setDlClient({ ...dlClient, zone: v })} placeholder="Zona Norte" />
          </div>
          <Field label="Dirección" value={dlClient.addr} onChange={v => setDlClient({ ...dlClient, addr: v })} placeholder="Calle, número" />
          <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2 mt-4">Transporte</label>
          {([
            { k: 'incluido' as const,      Icon: Receipt, t: 'Incluir Transporte',  d: 'Se agrega al total' },
            { k: 'pago_entrega' as const,  Icon: Coins,   t: 'Pago en Entrega',     d: 'Paga directo al chofer' },
          ]).map(o => (
      ...
      {/* Receipt modal */}
      {showReceipt && (
        <Modal onClose={() => setShowReceipt(null)} title="Ticket de Venta" Icon={Receipt}>
          <div className="bg-white border border-dashed border-slate-300 p-6 font-mono text-sm max-w-xs mx-auto">
            ...
            <div className="border-t border-dashed border-slate-300 mt-2 pt-2 text-[11px] space-y-0.5">
              <div className="flex justify-between"><span>Método:</span><span>{showReceipt.method}</span></div>
              {showReceipt.method === 'Efectivo' && <div className="flex justify-between"><span>Cambio:</span><span>{fmt(showReceipt.change)}</span></div>}
              {showReceipt.method === 'Mixto' && <>
                <div className="flex justify-between"><span>Efectivo:</span><span>{fmt(showReceipt.cashAmount)}</span></div>
                <div className="flex justify-between"><span>QR:</span><span>{fmt(showReceipt.qrAmount)}</span></div>
              </>}
            </div>
          ...
          <div className="flex justify-end gap-2 mt-4">
            <button onClick={() => window.print()} className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold border border-slate-200 rounded-lg hover:bg-slate-50">
              <Printer size={14} /> Imprimir
            </button>
            <button onClick={() => setShowReceipt(null)} className="flex items-center gap-1.5 px-5 py-2 text-sm font-bold bg-indigo-500 text-white rounded-lg">
...
function Modal({ children, onClose, title, Icon }: { children: React.ReactNode; onClose: () => void; title: string; Icon: ElementType }) {
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-[90%] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all">
            <X size={14} />
          </button>
        </div>
